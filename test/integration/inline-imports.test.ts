...
describe("inline-imports", () => {
  ...
  it("rewrites top-level typeof import()", () => {
    const t = fixFixture("inline-imports");
    const preFile = [...t.result.filesChanged].find(
      (f) => f.endsWith("pre-existing.ts"),
    );
    expect(preFile).toBeDefined();
    const content =
      t.project.getFileContent(preFile!);
    expect(content).not.toMatch(
      /typeof import\("/,
    );
    expect(content).toMatch(
      /typeof InternalModule\b/,
    );
  });
  ...
  it("adds namespace import statement", () => {
    ...
    const content =
      t.project.getFileContent(preFile!);
    expect(content).toMatch(
      /import type \* as InternalModule from/,
    );
  });
  ...
  it("resolves name conflicts", () => {
    const t = fixFixture("inline-imports");
    const conflictsFile = [
      ...t.result.filesChanged,
    ].find((f) => f.endsWith("conflicts.ts"));
    expect(conflictsFile).toBeDefined();
    const content =
      t.project.getFileContent(conflictsFile!);
    // InternalModule and InternalModule2 are taken,
    // so the alias should be InternalModule3
    expect(content).toMatch(
      /import type \* as InternalModule3 from/,
    );
    expect(content).toMatch(
      /typeof InternalModule3\b/,
    );
  });
  ...
  it("increments past all conflicts", () => {
    ...
    expect(content).not.toMatch(
      /import type \* as InternalModule\b[^3]/,
    );
    expect(content).not.toMatch(
      /import type \* as InternalModule2\b/,
    );
  });

  it("skips typeof import() inside TypeLiteral", () => {
    const t = fixFixture("inline-imports");
    const lazyFile = [...t.result.filesChanged].find(
      (f) => f.endsWith("lazy.ts"),
    );
    expect(lazyFile).toBeDefined();
    const content =
      t.project.getFileContent(lazyFile!);
  ...
  it("reuses existing namespace import", () => {
    const t = fixFixture("inline-imports");
    const nsFile = [...t.result.filesChanged].find(
      (f) => f.endsWith("existing-ns.ts"),
    );
    expect(nsFile).toBeDefined();
    const content =
      t.project.getFileContent(nsFile!);
