...
describe("applyTextChanges", () => {
  ...
  it("multiple non-overlapping changes (random order)", () => {
    ...
    expect(applyTextChanges(text, changes)).toBe("AAA BBB CCC DDD");
  });

  it("change at start of file", () => {
    const result = applyTextChanges("abc def", [makeChange(0, 3, "XYZ")]);
    expect(result).toBe("XYZ def");
  });

  it("change at end of file", () => {
    const result = applyTextChanges("abc def", [makeChange(4, 3, "XYZ")]);
  ...
  it("deletion (empty newText)", () => {
    const result = applyTextChanges("hello beautiful world", [
      makeChange(5, 10, ""),
    ]);
  ...
  it("replace with longer text", () => {
    const result = applyTextChanges("say hi to them", [
  ...
  it("replace with shorter text", () => {
    const result = applyTextChanges("say hello world to them", [
      makeChange(4, 11, "hi"),
    ]);
  ...
  it("many changes in a large file", () => {
    ...
    const expected = lines.map((l) => l.replace("line", "LINE")).join("\n");
